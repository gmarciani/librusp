...
static void *sndBufferizerLoop(void *arg) {
	...
	while (getConnectionState(conn) != RUDP_CON_CLOS) {
		...
		TimeoutObject timeoutobj = {.conn = conn, .sgm = sgm};

		TSegmentBufferElement *timeoutsgm = addTSegmentBuffer(conn->sndsgmbuff, sgm, RUDP_SGM_NACK, timeoutHandler, &timeoutobj, sizeof(TimeoutObject));
...
static void timeoutHandler(union sigval arg) {
	...
	clock_gettime(CLOCK_MONOTONIC, &(timeoutobj->elem->addtime));

	setTimer(timeoutobj->elem->timer, conn->timeo, 0);
